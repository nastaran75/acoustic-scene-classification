...
def build_densenet(input_shape=(None, 1, 66150), input_var=None, classes=15,
...
def dense_block(network, num_layers, growth_rate, dropout, name_prefix,nonL='rectify'):
    ...
    for n in range(num_layers):
        ...
        conv = bn_relu_conv(network, channels=growth_rate,
                            ...
                            name_prefix='hi',nonL=nonL)
        network = ConcatLayer([network, conv], axis=1,
                              name='D_join')
    return network


def transition(network, dropout,filter_size, name_prefix,conv_stride=2,pool_size=2,pad='valid',nonL='rectify'):
