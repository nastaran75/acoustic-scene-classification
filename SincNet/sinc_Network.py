...
def build_cnn(data_len,input_var = None, division=1):
  ...
  conv3 = batch_norm(lasagne.layers.DenseLayer(act2, num_units=256, W=lasagne.init.GlorotNormal(),
   nonlinearity=my_leaky_rectify))

  conv4 = batch_norm(lasagne.layers.DenseLayer(conv3, num_units=256, W=lasagne.init.GlorotNormal(),
   nonlinearity=my_leaky_rectify))

  conv5 = batch_norm(lasagne.layers.DenseLayer(conv4, num_units=256, W=lasagne.init.GlorotNormal(),
   nonlinearity=my_leaky_rectify))
