...
def train(depth,dataset_name,nonL,fold,division,augment,patience,representation,epochs,learning_rate,batch_size,dropout,growth_rate):
	...
	model_name = 'final_models/' + date + '_' + dataset_name +'-fold' + str(fold)+ '_wave_model_' + str(fold) \
	...
	train_log = log_dir + 'final_' + date+ 'fold_' + str(fold) +'_training_log.txt'
	validation_log = log_dir + 'final_' + date +  'fold_' + str(fold) +'_validation_log.txt'
	test_log = log_dir + date + 'final_' + 'fold_' + str(fold) +'_test_log.txt'
	...
	for epoch in range(epochs):
		...
		with open(train_log, 'a') as text_file:
			text_file.write("Epoch {} of {} fold: {} training_loss: {:.6f}  ,real_train_accuracy: {:.2f} %\n\n".format(epoch, epochs,fold,train_err, train_acc))

		with open(validation_log, 'a') as text_file:
			text_file.write("Epoch {} of {} fold: {} validation_loss: {:.6f}  ,real_validation_accuracy: {:.2f} %\n\n".format(epoch, epochs,fold,val_err, val_acc))
